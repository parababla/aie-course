...
def quality_flags_from_csv(file: UploadFile = File(...)):
    ...
    """
    Возвращает полный набор флагов качества из CSV-файла, включая новые флаги из HW03.
    """
    ...
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags_all = compute_quality_flags(summary, missing_df, min_missing_share=0.5)
