...
def report(
    ...
    min_missing_share: float = typer.Option(0.2, help="Порог доли пропусков."),
    ...
    # Список колонок, в которых слишком много пропусков
    too_many_mission_col_list = []
    for col in summary.columns:
        if col.too_many_missing:
            too_many_mission_col_list.append(col.name)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        # Меняем, т.к. теперь есть параметр title
        f.write(f"# {title}\n\n")
        ...
        # Добавляем две новые эвристики
        f.write(f"- Есть колонки, где все значения одинаковые: **{quality_flags['has_constant_columns']}**\n")
        f.write(f"- Есть дубликаты идентификатора: **{quality_flags['has_suspicious_id_duplicates']}**\n\n")
        # ============
        ...
        if too_many_mission_col_list:
            f.write(f"Колонки со слишком большим количеством пропусков: {', '.join(too_many_mission_col_list)} .\n\n")
        else:
            f.write("Колонок со слишком большим количеством пропусков не обнаружено.\n\n")
